...
fn options() -> Reader(Element(message), Context(message)) {
  ...
  case options.unique_keys(config.options) {
    Error(report) ->
      reader.return(
        html.div([core.classes(select_options_style)], [
...
fn group_source(
  ...
  source source: source.Resetable,
) -> Reader(Element(message), Context(message)) {
  use config <- reader.bind(get_config())

  let return = fn(element) {
    element.fragment([group_label(label), element])
    |> reader.return
  }

  case reset.unwrap(source), config.debug {
    Ok(source.Literal(value)), _debug ->
      group_value(label, value, source.initial_placeholder(source))
...
fn group_value(
  ...
  case keys, initial_placeholder {
    ...
    Error(report), _initial_placeholder ->
      reader.return(
        element.fragment([
          group_label(label),
          core.inspect([attr.class("p-3 text-red-800")], report),
...
fn find(
  label: Option(String),
  keys: List(Value),
) -> Reader(List(Value), Context(message)) {
  use config <- reader.bind(get_config())
  let words = option.map(config.applied_search, string_extra.words)
  use <- return(reader.return)
