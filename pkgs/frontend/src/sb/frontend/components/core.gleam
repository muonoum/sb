...
pub fn inspect(attr: List(Attribute(message)), value) -> Element(message) {
  let attr =
    list.append(attr, [
      attr.class("font-semibold font-mono text-wrap break-all"),
    ])

  html.pre(attr, [html.text(string.inspect(value))])
}
...
pub fn inline_value(value: Value) -> Element(message) {
  case value {
    ...
    value.Pair(key, _value) -> html.text(key)
...
pub fn page(content: List(Element(message))) -> Element(message) {
  html.div(
    [
      attr.id("sb-page"),
      classes([
        "fixed inset-0 top-(--header-height)",
