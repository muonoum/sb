...
pub fn update(model: Model, message: Message) -> #(Model, Effect(Message)) {
  ...
  case message {
    ...
    Receive(Error(report)) -> {
      let state = loadable.fail(report, value: None)
    ...
    Search(field_id:, string: "") -> {
      // TODO: Resett placeholder + bør det være mulig å søke med placeholder,
      // gjøre et valg, og så søke igjen for å gjøre flere valg?
      use state <- resolved_state(model)
...
fn get_is_loading(id: String) -> Reader(fn(Source) -> Bool, Context) {
  ...
  reader.return(field.is_loading(id, _, task.fields))
}
...
fn task_header() -> Reader(List(Element(Message)), Context) {
  // TODO: Vis antall felt med feil (hvis vi har layout er decode-feil skjult nå)
...
fn task_options() -> Reader(Element(Message), Context) {
  ...
  html.div([core.classes(["flex gap-3 items-center p-1 rounded-sm"])], [
    case debug {
      True -> enabled_option("hide debug", ToggleDebug, icons.eye_outline)

      False ->
        disabled_option("show debug", ToggleDebug, icons.eye_slash_outline)
    },

    case layout {
      True -> enabled_option("hide layout", ToggleLayout, icons.squares22)
      False -> disabled_option("show layout", ToggleLayout, icons.squares22)
    },
  ])
}
...
fn task_fields() -> Reader(Element(Message), Context) {
  ...
  case task.layout, layout {
    ...
    layout.Grid(areas:, ..), _use_layout -> grid_layout(areas)
  ...
  case field {
    Error(report) -> {
      ...
      html.div([core.classes(field_row_style)], [
        field_error(report),
        // TODO: Ser rart ut hvis vi har et gyldig felt med samme id siden
        // FieldContext(id) ikke blir plukket opp av duplikatsjekken.
...
fn hidden_field(content: Element(message)) -> Element(message) {
  // TODO: Vil vise skjulte felter i debug-modus, men er vanskelig å wrappe
  // innhold pga. 'last'-styling i field_row_style
  content
}
...
fn debug_sources(
  ...
  let attrs = list.append([attr.class("inline")], attr)

  case source {
    Error(report) ->
      html.li([attr.class("list-[square] px-4 mb-1 last:mb-0")], [
        core.inspect([attr.class("text-red-800 inline")], report),
      ])
...
fn field_kind(id: String, field: Field) -> Reader(Element(Message), Context) {
  ...
  let data_config = fn(source) {
    data.Config(
      source: reset.unwrap(source),
  ...
  case field.kind {
    kind.Data(source) -> data.field(data_config(source))
