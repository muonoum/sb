...
pub fn ok(v: v) -> State(Result(v, err), ctx) {
  return(Ok(v))
}

pub fn error(err: err) -> State(Result(v, err), ctx) {
  return(Error(err))
}
...
pub fn map_ok(
  ...
  map(state, result.map(_, mapper))
}

pub fn map_error(
  state: State(Result(v, a), ctx),
  mapper: fn(a) -> b,
) -> State(Result(v, b), ctx) {
  map(state, result.map_error(_, mapper))
}
