...
pub fn ok(value: v) -> Try(v) {
  state.ok(value)
}

pub fn error(report: Report(Error)) -> Try(v) {
  state.error(report)
}
...
pub fn replace(dict: Dict(String, Dynamic)) -> Props(Nil) {
  state.update(fn(_) { Context(dict:) })
}
...
pub fn main() {
  let decoder1 = {
    use source <- state.try(error_decoder())
    state.ok(reset.new(source, source.refs))
  }

  let decoder2 = {
    use source <- state.bind(error_decoder())
    state.return(reset.try_new(source, source.refs))
  }

  state.run(context: Context(dict.new()), state: decoder2)
  |> echo
}

fn ok_decoder() -> Try(Source) {
  state.ok(source.Literal(value.Null))
}

fn error_decoder() -> Try(Source) {
  state.error(report.new(error.Todo("error")))
}
