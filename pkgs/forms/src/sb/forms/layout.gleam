...
pub type Layout {
  ...
  Grid(results: Results, areas: List(List(String)))
}

pub fn decoder(results: Results) -> Zero(Layout, Nil) {
...
fn grid_decoder(results: Results) -> props.Try(Layout) {
  use areas <- props.get("areas", {
    decoder.from(decode.list(decode.list(decode.string)))
  })

  state.ok(Grid(results, areas:))
}
