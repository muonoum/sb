...
fn format_fields(fields: Dict(String, Field)) -> List(String) {
  ...
  let id =
    space_separated([
      theme_label(format_kind_name(field.kind)),
      theme_id(id),
    ])
...
fn format_data_kind(source: source.Resetable) -> List(String) {
  [space_separated([theme_label("source"), format_resetable_source(source)])]
}

fn format_text_kind(string: String) -> List(String) {
  [space_separated([theme_label("string"), string])]
}
...
fn format_multiple_choice(
  ...
  let options =
    space_separated([theme_label("options"), format_options(options)])

  let selected =
    space_separated([theme_label("selected"), format_multiple_selected(choices)])
...
fn format_single_choice(value: Option(Value), options: Options) -> List(String) {
  ...
  let selected =
    space_separated([theme_label("selected"), format_single_selected(value)])
...
fn format_resetable_source(source: source.Resetable) -> String {
...
pub fn format_source(source: Source) -> String {
  case source {
    source.Literal(value) -> format_value(value)
    source.Fetch(method:, uri:, headers: _, timeout: _, body:) ->
    ...
    source.Command(command: _, stdin: _) -> "TODO: Command"
  }
}
...
fn format_value(value: Value) -> String {
  case value {
    ...
    value.Pair(key, value) -> key_value(theme_string(key), format_value(value))
    value.Object(object) -> format_object(object)
  }
}

fn format_list(list: List(Value)) -> String {
  square_bracketed(comma_separated(list.map(list, format_value)))
}
...
fn format_object(pairs: List(#(String, Value))) -> String {
  curly_bracketed(
    space_separated({
      ...
      key_value(theme_string(key), format_value(value))
    }),
  )
}
...
fn format_options(options: Options) -> String {
  case options.sources(options) {
    [source] -> format_resetable_source(source)
    sources -> piped(list.map(sources, format_resetable_source))
  }
}
...
fn format_multiple_selected(selected: List(Value)) -> String {
  case selected {
    [] -> format_empty()
    list -> square_bracketed(space_separated(list.map(list, format_value)))
  }
}
...
fn theme_label(string: String) -> String {
...
// fn parenthesised(string: String) -> String {
//   surrounded(string, "(", ")")
// }
...
fn space_separated(list: List(String)) -> String {
  string.join(list, " ")
}

fn comma_separated(list: List(String)) -> String {
  string.join(list, ", ")
}
...
fn line_separated(list: List(String)) -> String {
  string.join(list, "\n")
}
